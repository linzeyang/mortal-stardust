...
class ExperienceSummaryModel(BaseModel):
    ...
    stage: str = Field(..., description="Stage summarized (stage1, stage2, stage3, or all)")
...
async def create_experience_summary_collection(db: Database):
    ...
    print(f"✅ Experience summaries collection setup completed")
...
async def setup_summary_analytics_views(db: Database):
    ...
    try:
        await db.create_collection("summary_analytics", viewOn="experience_summaries", pipeline=[
            {
                "$group": {
                    "_id": "$user_id",
                    ...
                    "avg_completeness": {"$avg": "$summary_data.summary_metadata.completeness"}
                }
            }
        ])
