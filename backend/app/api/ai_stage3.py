    ...
    try:
        ...
        if existing_solution:
            ...
            await db.solutions.update_one(
                {"_id": solution_id},
                {
                    "$set": {
                        ...
                        "lastFollowUpData": request.follow_up_data.dict() if request.follow_up_data else None
                    }
                }
            )
        else:
            ...
            solution_doc = {
                ...
                "stage2SolutionId": ObjectId(request.stage2_solution_id) if request.stage2_solution_id else None,
                "followUpData": request.follow_up_data.dict() if request.follow_up_data else None,
    ...
    try:
        ...
        if content and solution_doc.get("status") == SolutionStatus.COMPLETED:
            try:
                ...
                if isinstance(content.get("follow_up_plan"), str) and content["follow_up_plan"].startswith("encrypted:"):
                    content["follow_up_plan"] = decrypt_data(content["follow_up_plan"])
                if isinstance(content.get("progress_assessment"), str) and content["progress_assessment"].startswith("encrypted:"):
                    content["progress_assessment"] = decrypt_data(content["progress_assessment"])
        ...
        return {
            ...
            "stage2_solution_id": str(solution_doc["stage2SolutionId"]) if solution_doc.get("stage2SolutionId") else None
        }
...
async def process_follow_up_background(
    ...
    try:
        ...
        await db.solutions.update_one(
            ...
            {
                "$set": {
                    "adaptiveRecommendations": adaptive_result["adaptive_recommendations"],
