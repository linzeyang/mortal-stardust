...
class ConsentRequest(BaseModel):
    consent_type: str = Field(..., description="Type of consent (data_processing, analytics, marketing)")
...
class DataRequestRequest(BaseModel):
    request_type: str = Field(..., description="Type of request (export, delete, anonymize, rectify)")
    details: Optional[Dict[str, Any]] = Field(default=None, description="Additional request details")
    retention_policy: Optional[str] = Field(default="complete", description="Retention policy for deletion")
...
async def get_data_request_status(
    ...
    try:
        ...
        response_data = {
            ...
            'estimated_completion': data_request.get('estimated_completion', '').isoformat() if data_request.get('estimated_completion') else None,
            'completed_at': data_request.get('completed_at', '').isoformat() if data_request.get('completed_at') else None,
            'error_message': data_request.get('error_message')
        }
        
        # Add download link for completed export requests
        if (data_request['request_type'] == 'export' and 
...
async def request_account_deletion(
    retention_policy: str = Query(default="complete", description="Retention policy: complete, anonymize_retain"),
    current_user: dict = Depends(get_current_user),
    db: Database = Depends(get_database)
):
    """
...
async def get_audit_log(
    limit: int = Query(default=50, ge=1, le=100, description="Number of audit log entries to return"),
...
async def process_export_request(user_id: str, request_id: str, db: Database):
    ...
    try:
        ...
        logger.error(f"❌ Export request {request_id} failed for user {user_id}: {str(e)}")

async def process_deletion_request(user_id: str, request_id: str, retention_policy: str, db: Database):
    ...
    try:
        ...
        logger.error(f"❌ Deletion request {request_id} failed for user {user_id}: {str(e)}")
...
async def process_anonymization_request(user_id: str, request_id: str, db: Database):
    ...
    try:
        ...
        logger.info(f"✅ Anonymization request {request_id} completed for user {user_id}")
    except Exception as e:
        logger.error(f"❌ Anonymization request {request_id} failed for user {user_id}: {str(e)}")