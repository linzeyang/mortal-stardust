...
from ..models.solution import SolutionInDB, SolutionResponse, SolutionStatus
...
class SolutionRating(BaseModel):
    solution_id: str
    rating_percentage: int = Field(..., ge=0, le=100, description="Rating percentage from 0-100")
    feedback_text: Optional[str] = Field(None, max_length=2000, description="Optional feedback text")
    helpful_aspects: Optional[List[str]] = Field(default_factory=list, description="What was helpful about the solution")
    improvement_suggestions: Optional[List[str]] = Field(default_factory=list, description="Suggestions for improvement")
    would_recommend: Optional[bool] = None
    implementation_difficulty: Optional[int] = Field(None, ge=1, le=10, description="How difficult to implement (1-10)")
...
async def rate_solution(
    ...
    try:
        ...
        rating_doc = {
            ...
            "feedbackText": encrypt_data(rating.feedback_text) if rating.feedback_text else None,
        ...
        return RatingResponse(
            ...
            message="感谢您的评价！" + (
                "由于评分较低，我们将重新生成更好的解决方案。" if regeneration_triggered 
                else "您的反馈已记录，有助于改进我们的服务。" if rating.rating_percentage >= 70
                else "您的反馈已记录。"
            ),
...
async def get_rating_analytics(
    ...
    try:
        ...
        stats = result[0]
        success_rate = (stats["high_ratings"] / stats["total_ratings"] * 100) if stats["total_ratings"] > 0 else 0
...
async def regenerate_solution_background(
    ...
    try:
        ...
        experience_doc = await db.experiences.find_one({"_id": solution_doc["experienceId"]})
    ...
    try:
        ...
        await db.solutions.update_one(
            {"_id": ObjectId(solution_id)},
            {
                "$set": {
                    "isSuccessful": True,
                    "successRecordedAt": datetime.utcnow()
                }
            }
        )
