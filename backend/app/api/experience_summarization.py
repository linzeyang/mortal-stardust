...
router = APIRouter(prefix="/experience-summarization", tags=["experience-summarization"])

# Request/Response Models
class SummaryRequest(BaseModel):
    experience_id: str = Field(..., description="Experience ID to summarize")
    stage: str = Field(default="all", description="Stage to summarize (stage1, stage2, stage3, or all)")
...
async def get_experience_summary(
    ...
    try:
        ...
        summaries = await summarization_service.get_experience_summaries(user_id=user_id)
    ...
    try:
        ...
        for experience_id in request.experience_ids:
            try:
                ...
                logger.error(f"Error generating summary for experience {experience_id}: {str(e)}")
                errors.append({
                    "experience_id": experience_id,
                    "error": str(e)
                })
...
async def get_summary_analytics(
    ...
    try:
        ...
        recent_summaries = sorted(summaries, key=lambda x: x["created_at"], reverse=True)[:5]
        recent_activity = [
            {
