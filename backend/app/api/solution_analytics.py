...
class AnalyticsRequest(BaseModel):
    min_rating: int = Field(
        default=70, ge=30, le=100, description="Minimum rating threshold"
    )
    stage_filter: Optional[str] = Field(
        default=None, description="Filter by stage (stage1, stage2, stage3)"
    )
    time_range_days: Optional[int] = Field(
        default=None, ge=1, le=365, description="Analyze last N days only"
    )
...
async def get_analytics_history(
    limit: int = Query(
        10, ge=1, le=50, description="Number of analytics results to return"
    ),
    current_user: dict = Depends(get_current_user),
    db: Database = Depends(get_database),
):
    ...
    try:
        ...
        return AnalyticsHistoryResponse(
            analytics_history=[
                AnalyticsHistoryItem(**item) for item in analytics_history
            ],
...
async def get_analytics_by_id(
    ...
    try:
        ...
        analytics_result = await analytics_service.get_analytics_by_id(
            user_id=user_id, analytics_id=analytics_id
        )
...
async def get_quick_insights(
    ...
    try:
        ...
        stats = result[0]
        success_rate = (
            (stats["high_rated_count"] / stats["total_ratings"]) * 100
        ...
        if len(recent_ratings) >= 5:
            ...
            second_half_avg = sum(recent_ratings[mid_point:]) / (
                len(recent_ratings) - mid_point
            )
    ...
    try:
            ...
            comparison["results"].append(
                {
                    ...
                    "avg_rating": overview.get("rating_statistics", {}).get(
                        "average", 0
                    ),
                    "success_metrics": {
                        "consistency_score": result.get("analytics", {})
                        .get("effectiveness", {})
                        .get("consistency_score", 0),
                        "improvement_rate": result.get("analytics", {})
                        .get("effectiveness", {})
                        .get("improvement_rate", 0),
                    },
                }
            )
        ...
        if len(comparison["results"]) >= 2:
            sorted_results = sorted(
                comparison["results"], key=lambda x: x["created_at"]
            )
            ...
            comparison["trends"] = {
                ...
                "solution_count_trend": last_result["total_solutions"]
                - first_result["total_solutions"],
                "overall_improvement": (
                    "improving"
                    if last_result["avg_rating"] > first_result["avg_rating"]
                    else "stable"
                ),
            }
...
async def export_analytics(
    ...
    try:
        ...
        analytics_result = await analytics_service.get_analytics_by_id(
            user_id=user_id, analytics_id=request.analytics_id
        )
        ...
        if request.format.lower() == "json":
            ...
            writer.writerow(
                [
                    "Average Rating",
                    overview.get("rating_statistics", {}).get("average", 0),
                ]
            )
            writer.writerow(
                ["Max Rating", overview.get("rating_statistics", {}).get("max", 0)]
            )
            writer.writerow(
                ["Min Rating", overview.get("rating_statistics", {}).get("min", 0)]
            )
