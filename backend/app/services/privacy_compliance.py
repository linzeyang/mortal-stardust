...
class PrivacyComplianceService:
    ...
    async def record_consent(
        ...
        try:
            ...
            result = await self.collections['consent_records'].insert_one(consent_record)
            ...
            logger.info(f"✅ Consent recorded for user {user_id}: {consent_type} - {consent_record['status']}")
            return {
    ...
    async def get_user_consents(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            consents = await self.collections['consent_records'].find({
                'user_id': user_id
            ...
            for consent in consents:
                ...
                elif consent['created_at'] > consent_summary[consent_type]['created_at']:
                    consent_summary[consent_type] = consent
    ...
    async def export_user_data(self, user_id: str, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Export experiences
            experiences = await self.collections['experiences'].find({
                'user_id': user_id
            }).to_list(length=None)
            ...
            # Export solutions
            solutions = await self.collections['solutions'].find({
                'user_id': user_id
            }).to_list(length=None)
            ...
            media_files = await self.collections['media_files'].find({
                'user_id': user_id
            }).to_list(length=None)
            ...
            # Export summaries
            summaries = await self.collections['experience_summaries'].find({
                'user_id': user_id
            }).to_list(length=None)
            ...
            # Export analytics
            analytics = await self.collections['solution_analytics'].find({
                'user_id': user_id
            }).to_list(length=None)
            ...
            audit_logs = await self.collections['audit_logs'].find({
                'user_id': user_id,
    ...
    async def anonymize_user_data(self, user_id: str, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for exp in experiences:
                anonymized_exp = await self._anonymize_experience(exp, anonymization_map)
            ...
            anonymization_log = {
                ...
                'records_processed': len(anonymized_experiences) + len(anonymized_solutions)
            }
    ...
    async def delete_user_data(self, user_id: str, request_id: str, retention_policy: str = "complete") -> Dict[str, Any]:
        """Delete user data according to GDPR right to erasure"""
        try:
            logger.info(f"🔄 Starting data deletion for user {user_id} with policy: {retention_policy}")
            ...
            if retention_policy == "complete":
                ...
                collections_to_delete = [
                    'experiences', 'solutions', 'media_files', 
                    'experience_summaries', 'solution_analytics'
                ]
                ...
                user_result = await self.collections['users'].delete_one({'_id': ObjectId(user_id)})
                ...
                anonymization_result = await self.anonymize_user_data(user_id, request_id)
                ...
                deletion_summary['anonymization_id'] = anonymization_result.get('anonymous_id')
    ...
    async def create_data_request(
        ...
        try:
            ...
            data_request = {
                ...
                'created_at': datetime.utcnow(),
                'details': details or {},
                'estimated_completion': datetime.utcnow() + timedelta(days=30),  # GDPR 30-day limit
            ...
            return {
                'request_id': request_id,
                'status': 'pending',
                'estimated_completion': data_request['estimated_completion'].isoformat()
            }
    ...
    async def get_privacy_dashboard(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get data requests
            data_requests = await self.collections['data_requests'].find({
                'user_id': user_id
            }).sort('created_at', DESCENDING).limit(10).to_list(length=None)
                ...
                'data_requests': [
                    {
                        ...
                        'estimated_completion': req.get('estimated_completion', '').isoformat() if req.get('estimated_completion') else None
            ...
            logger.error(f"❌ Error generating privacy dashboard for user {user_id}: {str(e)}")
    ...
    async def _anonymize_experience(self, experience: Dict, anonymization_map: Dict) -> Dict:
        ...
        anonymized = {
            ...
            'content_type': 'text' if experience.get('content', {}).get('text') else 'multimodal',
    ...
    async def _anonymize_solution(self, solution: Dict, anonymization_map: Dict) -> Dict:
    ...
    async def _anonymize_demographics(self, user_id: str) -> Dict:
        ...
        demographics = {
            ...
            'age_range': self._get_age_range(user.get('dateOfBirth')) if user.get('dateOfBirth') else None
        }
    ...
    async def _get_data_usage_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for collection_name, collection in self.collections.items():
                if collection_name in ['users', 'consent_records', 'data_requests', 'audit_logs']:
                    continue
