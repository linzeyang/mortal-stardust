...
class SecureFileStorage:
    ...
    def __init__(self):
        self.base_dir = Path(settings.UPLOAD_DIR if hasattr(settings, 'UPLOAD_DIR') else "uploads")
        ...
        try:
            ...
            if file_type in self.allowed_extensions:
                if file_extension not in self.allowed_extensions[file_type]:
                    raise ValueError(f"File extension {file_extension} not allowed for {file_type}")
            ...
            if encrypt:
                ...
                subdir = file_type if file_type in ['audio', 'image', 'video'] else 'other'
                stored_path = self.base_dir / subdir / f"{file_id}{file_extension}"
            
            # Store file
            if encrypt:
                encrypted_data = encrypt_data(file_data.decode('latin-1') if isinstance(file_data, bytes) else file_data)
    ...
    async def retrieve_file(self, file_id: str, user_id: str) -> tuple[bytes, StoredFile]:
        ...
        try:
            ...
            if stored_file.user_id != user_id:
                raise PermissionError(f"User {user_id} does not have access to file {file_id}")
            
            # Read file
            file_path = Path(stored_file.stored_path)
            if not file_path.exists():
                raise FileNotFoundError(f"File data not found: {stored_file.stored_path}")
    ...
    async def cleanup_temp_files(self):
        ...
        try:
            ...
            for storage_dir in [self.base_dir / "audio", self.base_dir / "image", 
                              self.base_dir / "video", self.base_dir / "encrypted"]:
