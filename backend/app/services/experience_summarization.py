...
class ExperienceSummarizationService:
    ...
    async def generate_experience_summary(
        ...
        try:
            ...
            if not experience_data:
                raise ValueError(f"Experience {experience_id} not found for user {user_id}")
    ...
    async def _get_experience_data(self, user_id: str, experience_id: str) -> Optional[Dict]:
    ...
    async def _get_solutions_data(self, user_id: str, experience_id: str) -> List[Dict]:
        ...
        try:
            solutions = list(self.db.solutions.find({
    ...
    async def _generate_multimodal_summary(
        ...
        try:
            ...
            # Text summary
            summaries["text_summary"] = await self._generate_text_summary(summary_context)
            
            # Key insights extraction
            summaries["key_insights"] = await self._extract_key_insights(summary_context)
            ...
            if solutions_data:
                summaries["solution_effectiveness"] = await self._analyze_solution_effectiveness(
                    solutions_data
                )
    ...
    async def _generate_progress_summary(self, context: Dict) -> Dict[str, Any]:
        ...
        avg_rating = sum(s.get("rating", 0) for s in solutions) / max(total_solutions, 1)
    ...
    def _calculate_progress_trend(self, solutions: List[Dict]) -> str:
        ...
        first_half_avg = sum(s.get("rating", 0) for s in sorted_solutions[:mid_point]) / max(mid_point, 1)
        second_half_avg = sum(s.get("rating", 0) for s in sorted_solutions[mid_point:]) / max(len(sorted_solutions) - mid_point, 1)
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "key_emotional_insights": ["Emotional analysis requires manual review."]
                }
        except Exception as e:
    ...
    async def _analyze_multimodal_content(self, experience_data: Dict) -> Dict[str, Any]:
        ...
        if content.get("audio"):
            ...
            if isinstance(audio_data, list):
                analysis["media_summary"]["audio"] = {
                    ...
                    "total_duration": sum(item.get("duration", 0) for item in audio_data if isinstance(item, dict))
                }
    ...
    async def _analyze_solution_effectiveness(self, solutions_data: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            "overall_effectiveness": sum(data["avg_rating"] for data in stage_effectiveness.values()) / len(stage_effectiveness),
            "stage_effectiveness": stage_effectiveness,
            "improvement_suggestions": self._generate_improvement_suggestions(stage_effectiveness)
        }
    ...
    async def _generate_summary_metadata(self, summaries: Dict) -> Dict[str, Any]:
        ...
        progress_data = summaries.get("progress_summary", {}).get("overall_progress", {})
        ...
        summary_score += min(len(content_types) * 5, 20)  # Max 20 points for content variety
        ...
        emotional_progress = summaries.get("emotional_analysis", {}).get("emotional_progress", "")
    ...
    def _calculate_completeness(self, summaries: Dict) -> float:
        ...
        expected_sections = [
            "text_summary", "key_insights", "progress_summary",
            "emotional_analysis", "media_analysis"
        ]
        
        completed_sections = sum(1 for section in expected_sections if summaries.get(section))
    ...
    async def get_experience_summaries(
        ...
        try:
            ...
            summaries = list(self.db.experience_summaries.find(query).sort("created_at", -1))
