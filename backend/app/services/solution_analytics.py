...
class SolutionAnalyticsService:
    ...
    async def analyze_high_rated_solutions(
        ...
        try:
            ...
            analytics_result = {
                "overview": await self._generate_overview_analytics(high_rated_solutions),
                "patterns": await self._analyze_solution_patterns(high_rated_solutions),
                "effectiveness": await self._analyze_effectiveness_trends(high_rated_solutions),
                "content_analysis": await self._analyze_solution_content(high_rated_solutions),
                "user_feedback_analysis": await self._analyze_user_feedback(high_rated_solutions),
                "temporal_analysis": await self._analyze_temporal_patterns(high_rated_solutions),
                "recommendations": await self._generate_improvement_recommendations(high_rated_solutions),
                "metadata": {
    ...
    async def _get_high_rated_solutions(
        ...
        try:
            ...
            if stage_filter:
                pipeline.insert(-1, {
                    "$match": {"solution.stage": stage_filter}
                })
            ...
            for solution_data in solutions:
                try:
                    ...
                    if solution_data["solution"].get("content"):
                        solution_data["solution"]["content"] = await self.secure_data_service.decrypt_data(
                            solution_data["solution"]["content"], user_id
                        )
                    
                    # Decrypt experience content
                    if solution_data["experience"].get("content"):
                        solution_data["experience"]["content"] = await self.secure_data_service.decrypt_data(
    ...
    async def _generate_overview_analytics(self, solutions: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "rating_statistics": {
                ...
                "min": min(ratings),
                "max": max(ratings),
                "std_dev": round(statistics.stdev(ratings) if len(ratings) > 1 else 0, 1)
            },
            ...
            "stage_performance": {
                stage: {
                    ...
                    "success_rate": round((stage_distribution[stage] / len(solutions)) * 100, 1)
                }
                for stage in stage_distribution.keys()
            },
            "temporal_distribution": time_buckets,
            "top_rated_solutions": sorted(
                [{"solution_id": str(s["solution_id"]), "rating": s["rating"], "stage": s["solution"]["stage"]} 
                 for s in solutions], 
        ...
        try:
            ...
            for solution in solutions:
                ...
                solution_data.append({
                    ...
                    "experience_role": solution["experience"].get("role", "unknown")
                })
    ...
    async def _basic_pattern_analysis(self, solutions: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            "common_patterns": ["High user engagement", "Clear action steps", "Personalized recommendations"],
            "success_factors": ["Specific guidance", "Relevant resources", "Follow-up support"],
            "solution_types": dict(solution_types),
            "effective_approaches": ["Step-by-step guidance", "Resource provision", "Emotional support"],
            "user_role_insights": {
            ...
            "content_themes": ["Problem-solving", "Skill development", "Emotional wellness"]
        }
    
    async def _analyze_effectiveness_trends(self, solutions: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "peak_performance_period": max(monthly_trends.keys(), key=lambda k: monthly_trends[k]["avg_rating"]) if monthly_trends else None,
    ...
    async def _analyze_solution_content(self, solutions: List[Dict]) -> Dict[str, Any]:
        ...
        content_metrics["avg_content_length"] = round(total_length / len(solutions), 0) if solutions else 0
    ...
    async def _analyze_user_feedback(self, solutions: List[Dict]) -> Dict[str, Any]:
        ...
        feedbacks = [s.get("user_feedback", "") for s in solutions if s.get("user_feedback")]
    ...
    def _basic_feedback_analysis(self, feedbacks: List[str]) -> Dict[str, Any]:
        ...
        positive_words = ["good", "great", "excellent", "helpful", "useful", "clear", "effective"]
        improvement_words = ["improve", "better", "more", "clearer", "detailed"]
        
        praise_count = sum(1 for feedback in feedbacks for word in positive_words if word in feedback.lower())
        improve_count = sum(1 for feedback in feedbacks for word in improvement_words if word in feedback.lower())
        
        return {
            "praise_themes": ["Clarity", "Helpfulness", "Effectiveness"],
            "improvement_areas": ["More details", "Additional resources"],
            "overall_sentiment": "positive" if praise_count > improve_count else "mixed",
    ...
    async def _analyze_temporal_patterns(self, solutions: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "peak_performance_day": max(day_averages.keys(), key=lambda k: day_averages[k]) if day_averages else None,
            "peak_performance_hour": max(hour_averages.keys(), key=lambda k: hour_averages[k]) if hour_averages else None
        }
    
    async def _generate_improvement_recommendations(self, solutions: List[Dict]) -> List[Dict[str, Any]]:
        """Generate AI-powered recommendations for improving solution effectiveness"""
        try:
            # Prepare summary data for AI analysis
            summary_data = {
                "total_solutions": len(solutions),
                "average_rating": round(statistics.mean([s["rating"] for s in solutions]), 1),
                "stage_distribution": {},
                "user_roles": set()
            }
    ...
    async def _store_analytics_result(
        ...
        try:
            ...
            analytics_doc = {
                ...
                "metadata": {
                    "total_solutions_analyzed": analytics_result["metadata"]["total_solutions_analyzed"],
    ...
    async def get_analytics_history(
        ...
        try:
            cursor = self.db.analytics_results.find({
                "user_id": ObjectId(user_id),
                "analytics_type": "solution_analytics"
            ...
            async for doc in cursor:
                try:
                    ...
                    analytics_list.append({
                        ...
                        "summary": {
                            "total_solutions": decrypted_data.get("metadata", {}).get("total_solutions_analyzed", 0),
                            "avg_rating": decrypted_data.get("overview", {}).get("rating_statistics", {}).get("average", 0)
                        }
                    })
                except Exception as e:
    ...
    async def get_analytics_by_id(self, user_id: str, analytics_id: str) -> Optional[Dict[str, Any]]:
