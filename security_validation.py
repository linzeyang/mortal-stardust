...
class SecurityValidator:
    ...
    def log_result(self, check_name: str, status: str, message: str, level: str = "INFO"):
    ...
    def check_environment_variables(self) -> None:
            ...
            if var in os.environ:
                ...
                if len(value) < 10:
                    self.log_result(f"Environment Variable {var}", "FAIL", 
                                  "Value too short - security risk", "ERROR")
                else:
                    self.log_result(f"Environment Variable {var}", "PASS", 
        ...
        for var in recommended_vars:
            if var not in os.environ:
                self.log_result(f"Environment Variable {var}", "WARN", 
                              "Recommended variable not set", "WARNING")
    ...
    def check_jwt_security(self) -> None:
        ...
        if len(jwt_secret) < 32:
            self.log_result("JWT Secret Length", "FAIL", 
                          "JWT secret should be at least 32 characters", "ERROR")
        elif len(jwt_secret) < 64:
            self.log_result("JWT Secret Length", "WARN", 
                          "JWT secret should be at least 64 characters for maximum security", "WARNING")
        else:
            self.log_result("JWT Secret Length", "PASS", 
        ...
        if jwt_secret:
            ...
            if complexity_score >= 3:
                self.log_result("JWT Secret Complexity", "PASS", 
                              "JWT secret has good character variety")
            else:
                self.log_result("JWT Secret Complexity", "WARN", 
                              "JWT secret should include uppercase, lowercase, digits, and special characters", "WARNING")
    ...
    def check_encryption_configuration(self) -> None:
        ...
        if not encryption_key:
            self.log_result("Encryption Key", "FAIL", 
        ...
        try:
            ...
            if len(decoded_key) == 32:  # 256-bit key
                self.log_result("Encryption Key", "PASS", 
                              "AES-256 encryption key properly configured")
            elif len(decoded_key) == 16:  # 128-bit key
                self.log_result("Encryption Key", "WARN", 
                              "Using AES-128, consider upgrading to AES-256", "WARNING")
            else:
                self.log_result("Encryption Key", "FAIL", 
                              f"Invalid key length: {len(decoded_key)} bytes", "ERROR")
        except Exception:
            self.log_result("Encryption Key", "FAIL", 
                          "Encryption key is not valid base64", "ERROR")
    ...
    def check_database_security(self) -> None:
        ...
        if not mongo_uri:
            self.log_result("Database Connection", "FAIL", 
                          "MongoDB connection string not configured", "ERROR")
            return
        
        # Check for authentication in connection string
        if "@" in mongo_uri:
            self.log_result("Database Authentication", "PASS", 
                          "Database authentication configured")
        else:
            self.log_result("Database Authentication", "FAIL", 
                          "Database authentication not configured", "ERROR")
        
        # Check for SSL/TLS
        if "ssl=true" in mongo_uri.lower() or "tls=true" in mongo_uri.lower():
            self.log_result("Database Encryption", "PASS", 
                          "Database connection encryption enabled")
        else:
            self.log_result("Database Encryption", "WARN", 
                          "Consider enabling SSL/TLS for database connection", "WARNING")
            ...
            if os.path.exists(file_path):
                ...
                if permissions in ["600", "640"]:
                    ...
                    self.log_result(f"File Permissions {file_path}", "WARN", 
                                  f"Permissions could be more restrictive: {permissions}", "WARNING")
                else:
                    self.log_result(f"File Permissions {file_path}", "FAIL", 
                                  f"Insecure permissions: {permissions}", "ERROR")
            else:
                self.log_result(f"File Permissions {file_path}", "INFO", 
    ...
    def check_cors_configuration(self) -> None:
        ...
        if "*" in cors_origins:
            self.log_result("CORS Configuration", "FAIL", 
                          "Wildcard CORS origins detected - security risk", "ERROR")
            ...
            self.log_result("CORS Configuration", "WARN", 
                          "Localhost in CORS origins - remove for production", "WARNING")
        else:
            ...
            try:
                ...
                if limit_value <= 1000:
                    ...
                    self.log_result("Rate Limiting", "WARN", 
                                  f"Rate limit may be too high: {limit_value}", "WARNING")
            except ValueError:
                self.log_result("Rate Limiting", "FAIL", 
                              "Invalid rate limit value", "ERROR")
        else:
            self.log_result("Rate Limiting", "WARN", 
    ...
    def check_logging_security(self) -> None:
        ...
        if log_level.upper() in ["INFO", "WARNING", "ERROR"]:
            self.log_result("Log Level", "PASS", 
                          f"Appropriate log level: {log_level}")
        elif log_level.upper() == "DEBUG":
            self.log_result("Log Level", "WARN", 
                          "DEBUG logging may expose sensitive information", "WARNING")
        else:
            self.log_result("Log Level", "INFO", 
    ...
    def check_security_headers(self) -> None:
        ...
        for config_file in security_configs:
            if os.path.exists(config_file):
                found_config = True
                self.log_result("Security Headers", "INFO", 
                              f"Found security configuration: {config_file}")
                break
        
        if not found_config:
            self.log_result("Security Headers", "WARN", 
                          "No security header configuration found", "WARNING")
