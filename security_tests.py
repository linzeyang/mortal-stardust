...
class SecurityTester:
        ...
        for password in weak_passwords:
            try:
                ...
                if response.status_code == 422:
                    self.log_test(f"Weak Password Rejection ({password})", "PASS", "Weak password correctly rejected")
                else:
                    self.log_test(f"Weak Password Rejection ({password})", "FAIL", f"Weak password accepted: {response.status_code}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "access_token" in data:
                    ...
                    self.log_test("User Registration", "FAIL", "No access token returned")
            else:
    ...
    def test_jwt_security(self):
        ...
        try:
            ...
            if 'exp' in decoded:
                ...
                now = datetime.now()
                if exp_time > now:
                    self.log_test("JWT Expiration", "PASS", f"Token expires at: {exp_time}")
                else:
    ...
    def test_authorization(self):
        ...
        try:
            ...
            if response.status_code in [401, 403]:
                self.log_test("Unauthorized Access", "PASS", "Protected endpoint requires authentication")
            else:
                self.log_test("Unauthorized Access", "FAIL", f"Protected endpoint accessible without auth: {response.status_code}")
        ...
        if 'login' in self.tokens:
            ...
            try:
                ...
                if response.status_code == 200:
                    self.log_test("Authorized Access", "PASS", "Valid token grants access")
                elif response.status_code == 404:
                    self.log_test("Authorized Access", "SKIP", "Endpoint not found")
                else:
                    self.log_test("Authorized Access", "FAIL", f"Valid token rejected: {response.status_code}")
        ...
        for payload in sql_payloads:
            try:
                ...
                if response.status_code in [400, 422]:
                    self.log_test(f"SQL Injection Protection", "PASS", "Malicious input rejected")
                elif response.status_code == 404:
                    self.log_test(f"SQL Injection Protection", "PASS", "User not found (safe)")
                else:
                    self.log_test(f"SQL Injection Protection", "WARN", f"Unexpected response: {response.status_code}")
            ...
            try:
                ...
                if response.status_code in [400, 422]:
                    ...
                    if payload in str(data):
                        self.log_test("XSS Protection", "FAIL", "XSS payload not sanitized")
                    else:
    ...
    def test_rate_limiting(self):
        ...
        for i in range(10):
            try:
                ...
                if response.status_code == 429:  # Too Many Requests
                    self.log_test("Rate Limiting", "PASS", f"Rate limit triggered after {i+1} attempts")
                    break
        ...
        if failed_attempts >= 10:
            self.log_test("Rate Limiting", "WARN", "No rate limiting detected in 10 attempts")
    ...
    def test_data_encryption(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                found_sensitive = [keyword for keyword in sensitive_keywords if keyword in content]
                if not found_sensitive:
                    self.log_test("Data Exposure", "PASS", "No sensitive data in HTTP responses")
                else:
                    self.log_test("Data Exposure", "WARN", f"Potential sensitive data: {found_sensitive}")
